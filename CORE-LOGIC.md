...
8b. [집중 모드 (원본에 없는 기능)](#8b-집중-모드-원본에-없는-기능)
...
| `assets/js/suggest.js` | 175 | **수정 제안** — finding → 고칠 수 있는 제안 | 제안 규칙을 고칠 때 |
| `assets/js/writeback.js` | 174 | **수정본 xlsx** — 원본 양식 보존하며 셀 교체 | 워크백이 실패할 때 |
| `assets/js/worklist.js` | 56 | 집중 모드 진행 상태 | — |
| `assets/js/work-app.js` | 451 | 집중 모드 오버레이 제어 | 작업 화면 UI를 고칠 때 |
...
원본 사이트에는 없는, 이 클론에만 있는 기능이다. 학생·과목 단위로 하나씩 넘겨가며
제안을 확인하고, 되도록 그 자리에서 NEIS 재업로드용 xlsx까지 만들어주는 화면이다.
설계 근거는 `docs/superpowers/specs/2026-08-02-work-mode-design.md` 참조.

흐름: 파일 캡처(`work-app.js`) → 셀 주소까지 들고 파싱(`writeback.plan`) →
finding → 제안(`suggest.build`) → 진행 상태 복원(`worklist`) → 교사가 체크 →
적용(`suggest.apply`) → 원본 셀만 바꿔 재조립(`writeback.build`).

### 8b.1 제안 종류 (5가지)

`suggest.js:48` `mk()`이 만드는 제안 객체의 `kind`.

| kind | 대상 | 기본 | 비고 |
|---|---|:---:|---|
| `safe` | 유의어(후보 1개)·따옴표·줄바꿈·괄호영문 | 체크됨 | 자동 적용 후보 |
| `pattern` | 과거형 — `PAST_SORTED`에 있는 어미만 | 체크됨 | 추측 치환 없음 |
| `choice` | 유의어(후보 2개 이상) — 교사가 고름 | 해제 | 아래 참조 |
| `manual` | 특수기호 등 문맥이 필요한 것 — 위치만 표시 | 해제 | `to`는 항상 `null` |
| `cut` | 문장 단위 삭제 (바이트 줄이기) | 해제 | `suggest.cuts()`가 별도로 만듦 |

**`choice`가 따로 있는 이유** (`suggest.js:60-68` `fromCaution`) — 유의어의 `alt`
필드가 가운뎃점(`·`)을 두 가지 뜻으로 쓴다. `동영상 플랫폼·영상 창작자`에서는
후보를 가르는 구분자이고, `영상 제작·편집 프로그램`에서는 한 낱말 안에 든
가운뎃점이다. 프로그램은 이 둘을 구별할 방법이 없다. 그래서 `alt`에 가운뎃점이
있으면 후보가 여러 개일 가능성이 있다고 보고 무조건 `choice`로 내려 교사가
고르게 한다. `safe`(자동 체크)는 후보가 하나뿐일 때만이다.

### 8b.2 손대면 안 되는 것

- **`from`은 항상 `text.slice(span[0], span[1])`이다.** `finding.quote`는
  규칙마다 의미가 달라(R2·R9는 문맥 전체) 치환에 쓰면 문장이 통째로 날아간다
  (`suggest.js:6-7`).
- **`writeback.plan()`은 `xlsx-parse.js`의 `parseNeisRows`가 걷는 것과
  정확히 같은 분기·같은 순서로 걸어야 한다.** Task 4가 세 차례 다시 걸린
  이유다 — 처음엔 `writeback`이 행을 자기 규칙으로 걸었고 `xlsx-parse.js`는
  다른 규칙으로 걸었는데, 둘이 갈라지는 순간 리더가 화면에 보여주는 텍스트와
  워크백이 실제로 쓰는 셀 주소가 어긋나 기록이 조용히 훼손됐다. **원칙:
  `plan`은 리더가 보는 것과 정확히 같은 것을 봐야 한다.** 행 하나를 더 잡거나
  덜 잡는 실수는 화면에서는 안 보이고 NEIS에 재업로드해 다시 파싱했을 때만
  드러난다(`writeback.js:11-19` 주석). `getNo`·`isHeaderRow`·`isLikelyName`이
  각각 `xlsx-parse.js:338`·`:350`·`:359`의 `getStudentNo`·`isStdHeaderRow`·
  `isLikelyName`과 동일하다는 점을 코드 주석에 못박아 둔 것도 이 때문이다
  (`writeback.js:33·46·56`).
- **`writeback.build`는 인자가 3개다 — `build(workbook, replacements, planResult)`.**
  세 번째 인자를 빼먹기 쉬운데, 이게 있어야 각 제안 셀의 `extra`(그 텍스트에
  합쳐진 연속행 주소들)를 찾아 빈 문자열로 비운다(`writeback.js:132`,
  `:145-150`). 안 비우면 재파싱 때 리더가 그 연속행을 또 이어 붙여 읽어
  문장 꼬리가 중복된다. **이 모듈에서 가장 조용히 터지는 함정이다** — 두 인자로
  호출해도 에러 없이 파일이 만들어지고, 문제는 그 파일을 NEIS에 다시 올려
  재파싱했을 때만 드러난다.
- **과거형 패턴표에 없는 활용형을 추측으로 추가하지 말 것.** `했 → 하`
  단순 치환은 `작성했음 → 작성하음`처럼 말을 부순다.
- **`PAST_SORTED`(`suggest.js:44`)는 어미 길이 내림차순으로 런타임에 정렬한
  배열이다.** 매칭에는 `PAST_ENDINGS`(작성 순서)가 아니라 반드시 이 정렬본을
  써야 한다. `이었다`(계사 활용)가 `었다`보다 먼저 매칭돼야 하는데, 순서가
  뒤집히면 `반장이었다`가 `반장이음`이 된다. 새 어미는 `PAST_ENDINGS`에 넣기만
  하면 되고 직접 정렬할 필요는 없다 — 정렬은 코드가 한다. 다만 이 사실을
  모르고 `PAST_ENDINGS`를 직접 순회하는 코드를 새로 짜면 조용히 깨진다.
- **`work-app.js`는 `subject-app.js`·`career-app.js`보다 먼저 로드돼야 한다.**
  `career-app.js`가 `change` 핸들러 끝에서 파일 input을 비우기 때문이다
  (`work-app.js:6-9`).

### 8b.3 워크백 제외 상황

Task 9 기준으로 실제 거부되는 것은 두 가지뿐이다.

| 상황 | xlsx | 어디서 막나 | 이유 |
|---|:---:|---|---|
| 인쇄덤프 | X | `writeback.plan` (`writeback.js:107-113`) | NEIS 출력 보고서라 되올릴 양식이 아님 |
| 진로활동 묶음 형식 | X | `work-app.js`의 `exportFixed` (`:445-448` 단일 파일, `:466` 다중 파일) | `career-app.js`가 이 형식을 고정 열(0=번호, 1=성명, 3=특기사항, 4=희망분야)과 "희망분야" 라벨 특례로 자체 파싱하는데, `writeback`은 그 규칙을 모르고 `xlsx-parse.js`의 일반 열 탐지로 같은 파일을 다시 훑는다. 그 결과 희망분야 열의 실제 값이 통째로 빠지고 "희망분야"라는 라벨 글자만 본문에 남는 어긋남이 `fixtures/career-bundle.xlsx`로 확인됐다(Task 9). 화면 표시(작업 목록의 items)는 그대로 두되 **수정본 내려받기·고친 문장 복사만** 막는다(집중 모드 화면 경고는 `collect()`, `work-app.js:122` 참고) |
...
이 감지가 참이면 `collect()`(`work-app.js:122`)가 `writeback.plan`의
...
상태와 무관하게 항상 렌더링된다(`work-app.js:211`). 대신 창체에서 파일을
...
`worklist.js`는 학생당 진행 상태를 `{ done, edits, picks }`로 저장한다
...
```bash
node tests/suggest.test.js     # 제안 생성 + 적용 + 속성 테스트
node tests/writeback.test.js   # plan/build 구조 보존
node tests/worklist.test.js    # 진행 상태 저장/복원
```

속성 테스트(`suggest.test.js:163-191`)가 핵심이다. 텍스트에 제안을 만들어
켜서 적용한 뒤 재스캔해서, **그 규칙의 finding 개수가 줄어야 하고, 이전에
없던 규칙이 새로 생기면 안 된다**는 것을 확인한다. 제안이 문장을 망가뜨려
다른 규칙을 새로 건드리거나, 치환 후에도 원래 규칙이 그대로 남아 있으면
여기서 실패로 걸린다.
