...
(function () {
  ...
  function apply(text, suggestions) {
    ...
    // span[0] 이 같을 때만 순서를 따진다. cut 은 문장 전체를 지우는 상위집합
    // 조작이라 안에 든 제안을 삼켜야 하지만, 그 규칙을 모든 제안에 적용하면
    // 유의어가 괄호영문 안에 중첩될 때(유튜브(YouTube)) 교사가 고른 선택이
    // 조용히 버려진다 — 화면 체크는 켜진 채로 남아 알아챌 방법이 없다.
    // 그래서 cut 만 앞세우고, 나머지는 기존 순서(배열 순서)를 유지한다.
    live.sort(function (a, b) {
      if (a.span[0] !== b.span[0]) return a.span[0] - b.span[0];
      var ac = a.kind === 'cut' ? 0 : 1;
      var bc = b.kind === 'cut' ? 0 : 1;
      return ac - bc;
    });
